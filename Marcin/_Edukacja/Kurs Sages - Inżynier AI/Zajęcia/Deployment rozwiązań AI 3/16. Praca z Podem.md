...
```bash
kubectl cp sample.text NAZWA_PODA:LOKALIZACJA --namespace=mrybinski
```

Możemy Nazwe Poda wziąć  z kubectl get pods --namespace=mrybinski 




Tak jak wcześniej na zajęcia z kontenera używaliśmy komendy `exec`  tak samo tutaj Mamy też `kubectl exec`:
