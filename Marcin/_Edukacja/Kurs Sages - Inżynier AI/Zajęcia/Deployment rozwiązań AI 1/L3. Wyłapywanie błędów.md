...
1. Po drugie w `PostgresConnector` musielibyśmy zdefiniować dodatkową metodę, która sprawdza czy połączenie z bazą danych jest możliwe. Jeżeli nie jest możliwe to wtedy moglibyśmy rzucić naszym własnym wyjątkiem  wcześniej zdefiniowanym `PostgresConnectionNotAliveError`. 
2. Następnie w podstawowym kliencie `BasePostgresClient` musielibyśmy w metodach `read` i `write` tuż przed odczytaniem danych lub ich zapisem użyć metody z `connector`-a aby sprawdzić czy połączenie jest otwarte. 
