...
class CreditScoringModelNotFoundError(APIError):  
    def __init__(self, path: Path):  
        self._msg = f"Loading Credit Scoring Model from {path} location"        super().__init__(self._msg)
```
...
I zamieniamy nasze `raise e` na `raise CreditScoringModelNotFoundError(path)`. 
...
I potem zamiast `raise e` to `raise RedisConnectionError(host, port)`
...
I potem zamiast `raise` to `raise PostgresConnectionError(database, user, host, port)`

Sprawdźmy czy to zadziała. 

1. Spróbujmy popsuć ścieżke w ładowaniu modelu -> jak widzicie dostaliśmy w logach 
