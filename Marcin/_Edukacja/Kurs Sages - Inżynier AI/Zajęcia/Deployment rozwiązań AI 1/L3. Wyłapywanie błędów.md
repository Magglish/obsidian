...
Ale są też błędy, które mogą się pojawiać i ich pojawienie się jest zupełnie normalne. Dla przykładu:
...
decision_postgres_connector = DecisionPostgresClient(postgres_connector)
...
response = DecisionResponse(decision='ACCEPT')
```

To co teraz zrobiłem to zainicjowałem sobie połączenie do postgresa oraz clienta, który będzie obsługiwał wysyłanie requestów i response do bazy.

```python
decision_postgres_connector.write(request, response)
```

Działa.

A co jeżeli w trakcie działania naszego API z jakiegoś powodu straciliśmy połączenie z bazą danych?
...
I teraz będę próbował wysłać dane do Postgresa i otrzymam: