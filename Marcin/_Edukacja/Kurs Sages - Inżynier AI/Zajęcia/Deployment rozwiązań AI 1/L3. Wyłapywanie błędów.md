...
Teraz jak do tego można podejść w życiu codziennym, na produkcji: Jeżeli w waszym API wystąpi niespodziewany błąd, czyli zostanie obsłużony przez naszego "strażnika" czyli exception handler na `Exception` to musimy zrozumieć naturę naszego błędu. Jeżeli np. jest to błąd wskazujący na definitywny bug w naszym kodzie np. z jakiś przyczyn mamy w naszym kodzie dzielenie przez Zero i otrzymujemy `ZeroDivisionError`, to jest to ewidentnie bug, który musimy poprawić. I on koniec końcow, jeśli poprawimy nasz kod, nigdy nie powinien już wystąpić. Ale są też błędy  