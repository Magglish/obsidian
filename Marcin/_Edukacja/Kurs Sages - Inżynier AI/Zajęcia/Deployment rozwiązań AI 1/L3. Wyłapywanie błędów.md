...
Stwórzmy w folderze `src/service` folder `utils` i umieścimy tam nasze `errors.py`.
...
Zacznijmy od ładowania modelu - tam użyliśmy `FileNotFoundError` - w naszym przypadku `FileNotFoundError` jest jak najbardziej ok
