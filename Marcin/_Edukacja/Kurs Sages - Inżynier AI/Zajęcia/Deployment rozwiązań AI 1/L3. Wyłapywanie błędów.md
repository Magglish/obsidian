...
Włączmy nasze API. A następnie ubijmy redisa poprzez `docker ps` a potem `docker kill <<PIERWSZE_ZNAKI_ID>>` . Zobaczmy co się stanie. Wyślijmy requesta. I ku może Waszemu zdziwieniu wyszło na to, że jednak otrzymaliśmy odpowiedź z naszego API. Zobaczmy teraz logi. Tutaj zobaczycie teraz fajną rzecz, która wynika z zastosowania Background Tasks - jeżeli coś się popsuje w nich, to to wciąż nie blokuje działania naszego API. Tak jak widzicie odpowiedź została zwrócona, a błąd zadział się dopiero później w background tasku. 
