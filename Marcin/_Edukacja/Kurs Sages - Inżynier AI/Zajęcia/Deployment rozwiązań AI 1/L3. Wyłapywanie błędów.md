...
async def api_error_exception_handler(request: Request, exc: APIError):
    logging.exception(f"Error occured in API.", exc_info=True)
    return JSONResponse(
        status_code=500,
...
Dobra to teraz żeby zobaczyć sens tego exception handlera musimy zdefiniować sobie drugi exception handler, który niejako dopełnia to co chcemy zrobić.