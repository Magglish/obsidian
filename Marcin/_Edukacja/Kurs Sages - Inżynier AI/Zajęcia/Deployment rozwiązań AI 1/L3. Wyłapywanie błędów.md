...
3. Następnie w podstawowym kliencie `BasePostgresClient` musielibyśmy w metodach `read` i `write` tuż przed odczytaniem danych lub ich zapisem użyć metody z `connector`-a aby sprawdzić czy połączenie jest otwarte. 
