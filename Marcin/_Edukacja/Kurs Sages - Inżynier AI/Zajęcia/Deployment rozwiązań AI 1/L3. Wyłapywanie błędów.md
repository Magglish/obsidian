...
1. Po pierwsze musielibyśmy zdefiniować swój własny błąd, który mówiłby o utracie połączenia z baza danych.
2. Po drugie w `PostgresConnector` musielibyśmy zdefiniować dodatkową metodę, która sprawdza czy połączenie z bazą danych jest możliwe.
3. Następnie w podstawowym kliencie `BasePostgresClient` musielibyśmy w metodach `read` i `write` tuż przed odczytaniem danych lub ich zapisem użyć metody z `connector`-a aby sprawdzić czy połączenie jest otwarte. Jeśli nie jest, to moglibyśmy rzucić naszym własnym wyjątkiem 
