...
Ale są też błędy, które mogą się pojawiać i ich pojawienie się jest zupełnie normalne. Wyobraźmy sobie sytuację, że z jakiegoś powodu nasze połączenie z postgresem zostało przerwane. No nie wiem.. dajmy na to baza przestała istnieć, albo nagle dostęp do niej przez sieć jest niemożliwy. Może ktoś namieszał coś w konfiguracji sieciowej i teraz już nie jest ona dostępna - powodów może być cała masa. Zasymulujmy sobie to zdarzenie dodając do naszego endpointa po prostu wywołanie zamknięcia połączenia:

```python
postgres_connector.close()
```

I zobaczmy co się stanie. Włączmy API i wyślijmy requesta. I ku może Waszemu zdziwieniu wyszło na to, że jednak otrzymaliśmy odpowiedź z naszego API. Zobaczmy teraz logi. Tutaj zobaczycie teraz fajną rzecz, która wynika z zastosowania Background Tasks - jeżeli coś się popsuje w nich, to to wciąż nie blokuje działania naszego API. Tak jak widzicie odpowiedź została zwrócona, a błąd zadział się dopiero później w background tasku. 

Dostajemy wiadomość, że trafił się błąd nieobsłużony przez Nas i związane jest to właśnie z tym, że nie mamy połączenia z bazą danych - cursor nie działa. Pytanie co możemy z tym zrobić? W rzeczywistości to nie jest bug. Po prostu tak się może zdarzyć, że baza może nie być dostępna. Jak możemy obsłużyć tą sytuację? W 3 trzech krokach:

Ja teraz szybko to zaimplementuje, żeby tylko Wam pokazać przykład. Bo to docelowo w naszej implementacji nie zostanie:

1. Po pierwsze musielibyśmy zdefiniować swój własny błąd, który mówiłby o utracie połączenia z baza danych np. `PostgresConnectionNotAliveError`

```python
class PostgresConnectionNotAliveError(APIError):  
    pass
```

2. Po drugie w `PostgresConnector` musielibyśmy zdefiniować dodatkową metodę, która sprawdza czy połączenie z bazą danych jest możliwe. Jeżeli nie jest możliwe to wtedy moglibyśmy rzucić naszym własnym wyjątkiem  wcześniej zdefiniowanym `PostgresConnectionNotAliveError`. 

```python
def is_alive(self):  
    if self.connection.closed == 1:  
        raise PostgresConnectionNotAliveError()
```

3. Po trzecie - na końcu, w podstawowym kliencie `BasePostgresClient` musielibyśmy w metodzie `write` tuż przed odczytaniem zapisem danych użyć metody z `connector`-a aby sprawdzić czy połączenie jest otwarte. 

W ten sposób ten błąd nie trafił by do naszego "strażnika" i nie dostałbym poziomu CRITICAL, a po prostu trafiłby tutaj do exception handlera łapiący nasze błędy `APIError` i wtedy dostalibyśmy błąd o poziomie ERROR. Czyli innymi słowy w ten sposób możemy przenieść to co jest niespodzewanie, dzięki dalszej analizie błędu do czegoś co rozumiemy, jesteśmy w stanie obsłużyć i gdy następny raz zdarzy sie taki sam błąd, to doskonale wiemy z czym mamy doczynienia - co jest powodem tego błędu. I dzięki temu możemy zdecydowanie łatwiej przejść do dowiedzenia się dlaczego straciliśmy połączenie z Postgresem i spróbować to naprawić i rozwikłać.
