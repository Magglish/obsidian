...
Generalnie best-practises dla klucza w Redisie jest format `JSON`, czyli `request.json()`. Ale: 1) nie wszystko można zamienić na JSON. I w takiej sytuacji jedynym rozwiązaniem jest `pickle`-owanie. 2) Sprawa nawet jeżeli można to zamienić na JSONa, tak jak w naszym przypadku, to jednak te rozwiązanie ma pewną wadę:
