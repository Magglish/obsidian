...
async def decisions(request: DecisionRequest) -> DecisionResponse:  
    response = decision_redis_client.get(request)  
    ...
    decision_redis_client.set(request, response)  
    return response
```
...
async def decisions(request: DecisionRequest) -> DecisionResponse:  
    ...
    if response is not None:  
        print("Cache hit")  
        print(f"Elapsed = {(time.time() - start_cache)*1000:.8f} ms")  
        return response  
    print("Cache miss")  
    start_model = time.time()  
	decision = model.predict_decision(request.to_dataframe())[0]  
	response = DecisionResponse(decision=decision) 
    print(f"Elapsed = {(time.time() - start_model) * 1000:.8f} ms")  
    decision_redis_client.set(request, response)  
...
Mamy ogromną różnicę, praktycznie 10x krotną. To jest bardzo dużo. I biorąc pod uwagę, że wasze API będzie odpytywane przez wiele systemów czy inne osoby, to to jest bardzo istotne. Ok to wyrzućmy te wszystkie printy i inne niepotrzebne rzeczy.
