...
W `"bike:1"` wchodzi wtedy request.json(), a pole `mapping` uzupełni się w zależności od tego własnie jaki endpoint jest odpytany - czy to 

Bo jeżeli Wasze API zwracałoby tylko jeden rodzaj outputu, to Hash niepotrzebnie sprawe komplikuje i nie ma uzysku w pamięci.
