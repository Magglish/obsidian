...
async def decisions(request: DecisionRequest) -> DecisionResponse:  
    ...
    print("Cache miss")  
    response = DecisionResponse(decision=model.predict_decision(request.to_dataframe())[0])  
...
async def decisions(request: DecisionRequest) -> DecisionResponse:  
    ...
    return response
```
Mamy ogromną różnicę, praktycznie 10x krotną. Ok to wyrzućmy te wszystkie printy i inne niepotrzebne rzeczy.
