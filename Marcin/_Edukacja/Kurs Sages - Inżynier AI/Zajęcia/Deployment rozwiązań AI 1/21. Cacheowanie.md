...
```
   ...
   2. Możemy zamienić nasz obiekt na `json` poprzez `request.json()` i wrzucić go do bazy `client.set(request.json(), 2)` i jak widzicie dostajemy `True` udało się. Możemy to sprawdzić poprzez `client.get(request.sjon())`
