...
Jest rozwiązanie tych problemów, które mi w mojej pracy bardzo dobrze się sprawdza i rozwiązuje problem, jaki wskazałem przy JSONie i nie jest to obiekt zpicklowany, więc nie narusza zasad security itd. Sposób jest następujący - zamiast użyć `str(request)` używam `repr(request)`
