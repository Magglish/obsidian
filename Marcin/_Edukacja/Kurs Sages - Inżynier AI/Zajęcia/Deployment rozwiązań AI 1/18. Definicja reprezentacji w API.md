...
Włączmy API i sprawdźmy sobie `/redoc`, zobaczmy jak wygląda dokumentacja. Więc mamy 3 różne endpointy. które określają już precyzyjniej reprezentację na której nasze metody HTTP będą wykonywały akcje. Nie mamy ogólnej reprezentacji `predictions` i sterowania outputem. Tylko po prostu dany endpoint reprezentujący `Decisions`. Wymagane inputy i odpowiedź, która jest.
