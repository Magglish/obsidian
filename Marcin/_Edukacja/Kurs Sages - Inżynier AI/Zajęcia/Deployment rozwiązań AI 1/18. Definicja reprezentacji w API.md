...
`features: pd.DataFrame | pd.Series | np.array | list` i jeszcze obok jakiś parametr który steruje co chcemy dostać za odpowiedź. Może teraz przesadziłem z tym przykładem, ale u mniej doświadczonych programistów spotykam się z takimi rozwiązaniami, bo wygląda to fancy
