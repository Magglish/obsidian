...
Dodajmy sobie na dole `print(response_type)` żeby zobaczyć co otrzymamy teraz na wejściu. Przejdźmy sobie do `send_example_request.py` i tam musimy dodać jeden nowy argument czyli `params` oraz zmieńmy sobie wywołanie endpointa, bo teraz nazywa sie `predictions`

```python
params = {"response_type": "decisions"}

url = "http://localhost:8080/predictions"
response = requests.post(url=url, headers=headers, json=data, params=params)
```

Uruchomimy sobie API, wyślijmy requesta i zobaczmy co otrzymaliśmy
