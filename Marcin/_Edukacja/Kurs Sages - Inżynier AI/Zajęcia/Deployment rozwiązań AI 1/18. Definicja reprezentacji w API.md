...
Teraz czym jest ten `Annotated`? Pokaże Wam na prostszym przykładzie, załóżmy że jest tutaj jakiś prosty parametr `param`, któremu nadam `Annotated[str]`, czyli mówimy, że jest to typu param typu string, natomiast wszelkie pozostałe argumenty, to są metadane, które mogą nam coś mówić o t
