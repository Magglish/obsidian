...
7. Jaki wpływ dla inżynierów ML ma fakt, że protokół HTTP jest albo `stateless` albo `stateful`. Powiem Wam, że żaden. Niestety różnice pomiędzy tymi protokołami są na bardzo niskim poziomie - na poziomie nawiązywania połączeń - i są one transparentne dla użytkownika i developera. My nie odczujemy różnic w korzystaniu z API, czy w końcu ludzie ustalą czy to jest `stateful` czy `stateless`. Więc tym faktem bym się nie przejmował.
8. Teraz co ja rekomenduję? To zależy od dwóch czynników:
