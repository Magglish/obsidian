...
Tak jak się można spodziewać, nasze żądanie zostało sparsowane tak jak określiliśmy to w naszych type hintach. Widzimy, że `foreign_worker`, `present_employment_since`, `personal_status_and_sex` są teraz typem Enum, a nie stringiem. Problem jaki na pewno  napotkanie stosując pythonowe Enum jest następujący - pandasowy DataFrame nie pracuje na Enumach, tylko na bazowych typach danych jak `str`, `int`, `float`, albo numpy-owych typach danych albo ich własnych pandasowych. Jeśli wartości w kolumnie byłyby Enumami to po prostu rzuci Wam błędem. Dlaczego w tym przypadku nasz request przeszedł? Dzięki tej pozostałości w kodzie, która została nam po przetwarzaniu surowego requesta
