...
9. Zresetujmy API już normalnie i zobaczmy co się zmieniło. Wykonajmy requesta z jednym polem więcej - i okazuje się, że dodatkowe pola nie są dozwolone. Można się teraz zapytać po co to zrobiliśmy? API działało poprawnie bez tego parametru. Powody są dwa: 
	...
	2. Po drugie, w zależności od fazy rozwoju waszych implementacji, może być tak, że np. cechy które przekazujemy do modeli się często zmieniają i to też wymaga po naszej stronie dostosowania API do nowej struktury danych. Może być tak, że tak jak tutaj mamy `send_example_request.py` zdefiniowałem sobie strukturę danych, którą wysyłam do API. 