...
9. Zresetujmy API już normalnie i zobaczmy co się zmieniło. Wykonajmy requesta z jednym polem więcej - i okazuje się, że dodatkowe pola nie są dozwolone. Można się teraz zapytać po co to zrobiliśmy? API działało poprawnie bez tego parametru. Powód jest prosty: pamiętacie jak mówiłem Wam, że API jest pewnego rodzaju kontraktem pomiędzy naszym serwisem a klientem/innym serwisem, który z niego korzysta - wysyłając dane w odpowiednim formacie (http://0.0.0.0:8080/redoc) otrzyma on od Nas dane też w odpowiednim formacie. Jeśli ja jako użytkownik mogę przekazać do API więcej niż to wynika z dokumentacji, to powiem Wam, że to wprowadza chaos w korzystanie z Waszego API. 