...
class BaseModelExtraFieldsForbidden(BaseModel):
    class Config:
        extra = "forbid"
```
...
Dlatego, że ten parametr `use_enum_values` jest nam tylko potrzebny w tym obiekcie, w `response.py` nie jest on potrzebny bo podczas zwracania odpowiedzi FastAPI sam to parsuje "pod spodem" i tam zawsze są brane warto `enum_values` jest używane.
