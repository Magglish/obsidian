...
1. `age` podobnie jak `installment_rate_in_percentage_of_disposable_income`. Tutaj umieszczam tylko `ge=18` i `le=65` czyli innymi słowy dopuszczamy w ogóle wykonywanie predykcji dla osób w wieku produkcyjnym. Na potrzeby kursu damy tutaj od 18 do 65 roku życia, chociaż prawidłowo moim zdaniem powinno być tylko `ge=0`, bo ta walidacja ma sprawdzić poprawność wejścia, ktoś może mieć 5 lat i np. 110 lat, ale nie może mieć mniej niż 0 lat. Natomiast obecne prawo u Nas w Polsce dopuszcza 18 lat, a polityka kredytowa banku może np. odrzucać z góry osoby powyżej 65 roku życia. Takie uzależnianie MLowej implementacje 