...
Tak jak się można spodziewać, nasze żądanie zostało sparsowane tak jak określiliśmy to w naszych type hintach. Widzimy, że `foreign_worker`, `present_employment_since`, `personal_status_and_sex` są teraz typem Enum, a nie stringiem. Ale nie mamy się o co martwić, bo fajne jest też to, że Enum jest obsługiwany przez Pandasa. Odpalmy ten kawałek kodu w konsoli, żeby to zobaczyć:

```python
features = pd.DataFrame({key: [value] for key, value in request.dict().items()})
```

I podejrzyjmy sobie wartość w kolumnie `personal_status_and_sex`

```python
features['personal_status_and_sex'].iloc[0]
```

Po prostu Pandas w momencie przetwarzania napotyka Enuma i bierze z niego wartość:

`features['personal_status_and_sex'].iloc[0].value`

Dalej kod już zadziała, ale musimy poprawić nasz Response, dlatego, że tam w type hintach określiliśmy, że decision jest typu `Decision`, wiec możemy wprost przekazać `decision`, czyli
...
def to_dataframe(self) -> pd.DataFrame:  
    ...
    Returns: Pandas data frame with request data.  
...
Ok to teraz wyjaśnienie po co to zrobiliśmy?
...
Teraz jest OK. Spójrzmy jeszcze raz na okno API i na okno z requestem. Tutaj zakomentowałem na chwilę tą linijkę kodu, bo odpowiedź, która przyszła z API jest na tą chwilę zwykłym stringiem, dlatego `request.json()` nie zadziałała, bo odpowiedź nie jest `JSO`