...
8. Wróćmy do naszego `PredictRequest` w `requests.py` i zdefiniujmy go sobie. Pydantic pozwala nam konfigurować nasze klasy poprzez zdefiniowanie wewnątrz nich takiej wewnętrznej klasy `Config`, która ustawia nam zachowanie tej danej klasy. Ustawimy sobie w niej 2 wartości:
	...
	2. `use_enum_values` - drugi parametr, który oznacza, że te dane które będą do nas przychodzić będą automatycznie transformowane na wartości z Enumów
9. Zresetujmy API już normalnie i zobaczmy co się zmieniło. Wykonajmy requesta z jednym polem więcej - i okazuje się, że dodatkowe pola nie są dozwolone. Można się teraz zapytać po co to zrobiliśmy? API działało poprawnie bez tego parametru. Powody są dwa: 
	1. pamiętacie jak mówiłem Wam, że API jest pewnego rodzaju kontraktem pomiędzy naszym serwisem a klientem/innym serwisem, który z niego korzysta - wysyłając dane w odpowiednim formacie (http://0.0.0.0:8080/redoc) otrzyma on od Nas dane też w odpowiednim formacie. Jeśli ja jako użytkownik mogę przekazać do API więcej niż to wynika z dokumentacji, to powiem Wam, że to wprowadza chaos i zamieszanie w korzystanie z Waszego API i ja osobiście bym się zastanawiał - co to znaczy, że ja mogę wysłać do API więcej danych niż wymaga? Czy to jakoś potem wpływa na działanie API? Po drugie może być tak, że 