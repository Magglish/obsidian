...
 Przechodzimy sobie do kolejnego tematu jakim  testowanie poprawności naszego API w przypadku testowania będziemy mówić o dwóch wymiarach przede wszystkim i w wymiarze jej implementacyjnym czy Innymi słowy czy nasze API zwraca to czego się po nim spodziewamy oraz też w wymiarze obciążeniowym to znaczy będziemy sprawdzać Jak szybko nasze API jest w stanie zwrócić odpowiedzi pod różnymi wariantami obciążenia czyli Innymi słowy jak Abi działa kiedy jest wysłane tylko jeden Rik ust na sekundę kiedy może wysłanych jest tych requestów 5 i tak dalej to jest bardzo ważny etap który wynalazek przynajmniej przedłużeniem żeby mieć ogólny pogląd na to jak nasze API będzie zachowywał się pod obciążeniem na produkcji

  

 testowania kodu było już omawiane na poprzednich zajęciach z promowania w pythonie jeden i dwa.  ja w tym module będę Oczywiście nawiązywał do pewnych pewnych pojęć które wtedy poznaliście ale osadzając jej całkowicie w kontekście API. 

  

 od razu wam powiem że pisanie testów API moim zdaniem jest trudniejsze niż otestowywanie implementacji naszych kodów jest to powiązane z tym że w API dzieje się dosyć sporo tak to widzicie nasze requeesy są odpowiednio przekształcane na pajdatikowy obiekt tutaj mamy sprawdzenie czy jest odpowiedź w cashu potem wykonanie modelu potem zapisanie tych danych do bazy zatem w ramach jednego endpointu dzieje się dosyć sporo. W dodatku każdy API ma dużo oczywiście elementów wspólnych czy to będzie API związane z tak tutaj w tym przypadku modelami oceniającymi ryzykorytowe ale też są elementy wspólne w przypadku Papi do modelu tekstowym uczy się przekonaliśmy na swojej masy ćwiczeniach ty też API to przetwarzania obrazów Jednakże wciąż są pewne różnice. Niestety też pisali niektórych testów jest dosyć czasochłonne dlatego tym bardziej jeśli ma się z tym kontakt po raz pierwszy bo trzeba zwrócić uwagę na bardzo wiele rzeczy i niestety ale też dopiero przyjdą wtedy kiedy taki test się trochę napisze kiedy nabierze się takie intuicji co warto przetestować jeszcze w ramach danego danej funkcji Czy ten implementacji temat nie jest łatwy powiem szczerze od razu.

  

Dodatkowo trudnością w testowaniu API z zaimplementowanym modelem jest fakt że nasz model może być niedeterministyczny w szczególności w przypadku kiedy będziemy implementować API dla modeli tekstowych generatywnych które mogą zwrócić dosłownie dowolny tekst. Nawet jeżeli pracujemy z bardziej klasycznymi algorytmami w klasycznym problemie jak na przykład regesta Logistyczna która za każdym razem zwróci nam ten sam wynik jeżeli podamy mu te same parametry Ale problem jest drugi taki że nasz model może się zmienić może to być i nawet zasady Logistyczna z innymi parametrami Czy zmiana algorytmu na drzewa decyzyjne i gdybym usta cokolwiek innego i wtedy dla danego wejścia zmieni się wynik dlatego też takie testy które porównują czy dany  czy dany wynik z modelu jest równa jakiś tam konkretnej wartości po prostu  będą nieużyteczne dlatego też do testowania API  który ma w sobie model masz learningowi trzeba podejść w pewien specyficzny sposób
