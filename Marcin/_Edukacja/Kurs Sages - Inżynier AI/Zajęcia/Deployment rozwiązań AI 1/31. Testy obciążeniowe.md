...
2. throughput - Druga z nich to jest throughput Czyli po polsku można by to nazwać przepustowość,  czyli ile requestów jesteśmy w stanie przetworzyć w jakiejś jednostce czasu minucie sekundzie godzinie w zależności od tego co was interesuje Przeważnie jest to liczone zawsze ilość requestów na sekundę.
    

  

Oczywiście z jednym metryki można przejść na drugą bo w naszym przypadku lata jest i w wysokości 2 ms oznacza 500 requestów na sekunde I vice versa. Generalnie patrzy się zawsze na te dwie metryki Łącznie.

  

W swojej pracy na co dzień w zależności od tego gdzie będzie robić API na pewno będziecie mieli  jakieś zamawiające do tego w jakim czasie chcemy zacząć odpowiedź 

1. Wiadomo - nikt nie lubi czekać 
    
2. Jest taka że jeżeli coś działa Naprawdę długo to jest to pewien sygnał może to tak żeby zastanowić się czy nie ma w naszym kodzie jakiejś kiepskiej implementacji która ten czas niepotrzebnie wydłuża.
    
3.  druga sprawa jest taka że inne systemy korzystające z naszego API też spowolnią jeżeli Odpowiedź z naszego API trwa dosyć długo. I też Apis wracając do odpowiedzi po długim czasie może wpłynąć na to jak produkt oparta wasz model będzie finalnie wyglądał Żebyście mogli to sobie łatwiej wyobrazić to podam dwa przykłady: Wyświetlanie reklam: tak naprawdę wyświetlanie reklam opiera się o taki o taki system aukcyjny w którym każda reklama jest oceniana poza tym tego czy wy w nią klikniecie Bądźmy wyświetlicie bądź będziecie w nią jakąś interakcję i w takim systemie używane są tego żeby ocenić jakość reklam i proponując do tego że wy nie klikniecie reklam konkurencji między sobą są setki  jak i tysiące I dla każdej z nich musi być ona predykcja więc w takich warunkach modele Musisz działać naprawdę szybko tej można mówić o ograniczeniach rzędu  kilku bądź kilkunastu milisekund perpedycja. U mnie w firmie Mamy mądre którym zwraca predykcję od 15 do 30 sekund Czy można to to przyspieszyć? Niestety nie. Jak ta sytuacja masz rozwiązać od strony huaweia i interakcji użytkownika z tym modelem tak naprawdę w momencie kiedy on z niego korzysta To dostaje informację zwrotną że Predykcja z modelu są przygotowywane i dostanie on powiadomienie w aplikacji o tym że jest już to gotowe po jakiś tam czasie. 
    
4. Więc generalnie badanie szybkości waszego API decyduje naprawdę o wielu aspektach i warto to sprawdzić.
    

  
  

Testo obciążeniowe będziemy wykonywać dzisiaj oraz na zjeździe 3 dzisiaj Chciałbym tylko żebyście zobaczyli z jakiego biblioteki byśmy korzystać Jakie ona może na wygenerować wyniki i tak dalej Natomiast te wyniki nie będą miarodajne. Dlaczego? Dlatego że Pamiętajcie że mamy na razie Apis grillowany lokalnie system produkcyjny  będzie inne pod względem procesorów i pamięci RAM zatem tak naprawdę najlepiej testo bierzemy przeprowadza się wtedy kiedy wdroży się już pewną wersję API na środowisko dev czy preprod, Które jest które powinno być kopią produkcji I Narnię prowadzi testy obciążeniowe.  Dzisiaj zrobimy te testy lokalnie także mówię żebyście zobaczyli z czego skorzystamy Jak wyglądają te wykresy z analizy i tak dalej a na trzecim wtedy skupimy się stricte na tym żeby zrobić to “po bożemu”. 
