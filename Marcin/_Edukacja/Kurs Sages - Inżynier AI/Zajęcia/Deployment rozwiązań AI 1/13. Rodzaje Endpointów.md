...
2) po drugie - w odpowiedzi, którą otrzymujemy od naszego endpointu widzimy, że mamy nagłówki z metadanymi a w nich jest warto

3. `POST`
4. `PUT`
5. `PATCH`
6. `DELETE`
7. rzadko spotykane `OPTIONS`, `HEAD`, `TRACE` oraz `CONNECT`