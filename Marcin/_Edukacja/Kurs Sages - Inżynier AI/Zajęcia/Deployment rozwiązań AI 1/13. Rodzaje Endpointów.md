...
Pytanie jakie moża Wam teraz powstać to to ,czy można wysłać zapytanie typu `POST` z przeglądarki? Tak jak robiliśmy to wcześniej z `GET`em i wyświetlaniem zawartości stron internetowych. Niestety odpowiedź brzmi nie. Defaultowo przeglądarka wysyła żądania typu `GET` i nie można ich zmienić. W dodatku dane, które przychodzą do `POST` nie są zawarte w URL
