...
To co widać na zdjęciu to treść błędu z jednego z naszych serwisów działających na produkcji na chmurze Google'a, która jak widzicie brzmi: "There was a problem opening a stream. Try turning on DEBUG level logs to see the error". I jeszcze bardzo ważne jest to, jaka biblioteka to rzuciła: `google.api_core`, która stanowi trzon wszystkich bibliotek Pythonowych, które pozwalają wam na interakcje z usługami w Google'u. Ten log który Wam pokazuje wystąpił wystąpił podczas zapisywania danych do BigQuery. Ale spotkałem się z nim jeszcze potem w kilku innych miejscach. Co możemy zrobić w takiej sytuacji? 
