...
Dalej mamy wywołanie naszego modelu `model = CreditScoringModel(path=Path("models/classifier.pkl"))`.  Czy mamy tutaj logi? Nie mamy. Czy według nas jest to istotna rzecz, żeby zlogować? Moim zdaniem tak. Model w API to podstawowy komponent działania naszego modelu, jeżeli 
