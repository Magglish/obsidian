...
12. Jaka jest zaleta podejścia REST? To, że przy podejściu REST, praktycznie wystarczą nam tylko i wyłącznie 4 metody z protokołu HTTP - `GET`, `POST`, `PUT`, `DELETE` aby móc wykonać wszystkie możliwe czynności na danej reprezentacji. W sytuacji stosowania podejścia RPC, musielibyśmy definiować każdy endpoint jako oddzielną czynność. Teraz ktoś może powiedzieć, że "ale przecież my i tak i tak musimy implementować obsługę metod `GET`, `POST`, `PUT`, `DELETE` w naszym API - nie unikniemy tego". Tak jak mamy to w naszym API - zdefiniowaliśmy endpoint `predict_decision`, który notabene jest niezgodny ze sztuką REST ale to poprawimy potem, a ta funkcja obsługuje metode `POST`. Gdybym chciał obłużyć `GET` to i tak musiałbym dodać tutaj nową funkcję i napisać kod, który obsłuży metodę `GET`. Czyli praktycznie to samo, jak w RPC. Więc o jakiej zalecie tutaj mówimy?   
13. 
