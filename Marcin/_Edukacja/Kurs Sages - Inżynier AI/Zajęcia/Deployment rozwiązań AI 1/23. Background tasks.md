...
# TODO: Do sprawdzenia
Ile masz watkow bo jednak one sa uruchamiane na innych wątkach i moze uda Ci sie "zapchac" Je wysylajac wiele requestow?

Background Tasks spawnują Lightweight Processes

1. https://stackoverflow.com/questions/10484355/what-is-the-difference-between-lightweight-process-and-thread
2. https://www.thegeekstuff.com/2013/11/linux-process-and-threads/
3. Odpal sobie `ps -o nlwp <<PID_API>>` żeby zobaczyć ile wątków/LWP używa teraz process oraz `watch ps -o thcount <<PID_API>>` aby zobaczyć w czasie rzeczywistym jak to się zmienia 
4. Odpal `htop`, filtruj po `src/` i kliknij `F5 Tree` aby rozwinac do drzewka żeby zobaczyć parent processy i child processy 
