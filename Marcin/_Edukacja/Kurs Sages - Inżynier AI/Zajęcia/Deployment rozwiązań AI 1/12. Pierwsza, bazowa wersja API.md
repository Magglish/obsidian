...
### JEST ZMIANA KONCEPCJI. NIE KODZISZ TEGO KROK PO KROKU TYLKO JUZ JEST W REPO WIEC TE OMÓWIENIE MUSISZ ZMIENIĆ
...
Następnie te API musimy uruchomić w trybie Debugera - niestety jeżeli uruchomimy to z poziomu terminala, to uruchomi się taki ubogi pythonowy `pdb`, który jest kiepski. Jeżeli uruchomimy to z IDE to dostaniemy coś znacznie lepszego, więc w tym przypadku polecam kliknąć tę strzałke obok `if __name__ == "__main__":` i wybrać `Debug 'main'`.
...
Ok. Mamy stworzone pierwsze API - jak widzicie, które działa w kilku linijkach kodu. To teraz wyjaśnimy sobie czym są te endpointy, które w pythonie przyjmują postać funkcji, zmodyfikowanych dekoratorem `@app.post` i `@app.get`. 
