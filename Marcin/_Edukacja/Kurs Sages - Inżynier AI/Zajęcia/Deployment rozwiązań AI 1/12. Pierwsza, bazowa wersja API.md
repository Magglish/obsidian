...
Wynika To przede wszystkim z dwóch rzeczy,  nie będę aż tak wnikał w szczegóły wrócę do tego pod koniec zjazdu jak już poznacie więcej tutaj rzeczy związanych z tym jak was a wiedziała natomiast musi wiedzieć o tym że:

1.  generalnie takie przetwarzanie baczowe czyli kiedy wasze API przetwarza w ramach jednego żąda,  jednej wysyłki danych  wiele różnych przypadków nie jest takie proste  i jest tam wiele problemów które trzeba zaadresować.  na przykład zakładamy że  nasze API przyjmuje minimum 10 takich wniosków kredytowych.  Jak napisać mechanizm ich kolejkowania?  A co jeżeli przyjdzie pięć takich wniosków kredytowych a potem długo długo nic? Mamy czekać  jakiś określony czas czy może po prostu czekać aż dopiero 10 wniosków przyjdzie?  takie czekanie  może  opóźnić działanie innych serwisów które czekają na wynik z naszego API?  natomiast taki Bach Processing jest implementowany w sytuacji kiedy modele naprawdę bardzo długo działają  i sensowne jest to aby taką dodatkową logikę kolejkowania,  czekania  i przetwarzania niepełnych paczy zaimplementować dlatego że zysk z ich prezentacji jest spory w postaci  szybszego  przetwarzania danych. I takie podejście najczęściej stosowane jest w przypadku sieci neuronowej dlatego że koszt  obliczeniowy przypuszczenia przez sieć pojedynczego przypadku No jest dosyć  więc w przypadku sieci normowych możecie spotkać się z tym że te przetwarzanie baczowe wielu requestów Na raz może być zaimplementowane.  ale w przypadku  innych prostszych modeli po prostu jedno jeden request przetwarza jeden przypadek.
...
Krótko mówiąc wasze API przeważnie będzie przetwarzać w ramach jednego requesta jeden przypadek.  implementacja baciowego przetwarzania jest trudna i ma wiele innych trudnych wyzwań i naprawdę to musi być uzasadnione żeby się w to bawić w postaci tego że faktycznie te batchowe przetwarzanie istotnie przyśpieszy wasze API.  ale w 90% w jedno zapytanie do API = jedna predykcja dla jednego przypadku będzie dla Was w zupełności wystarczająca.
