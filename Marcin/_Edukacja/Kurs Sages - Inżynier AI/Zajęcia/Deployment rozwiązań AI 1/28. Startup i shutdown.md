...
Dlaczego o tym mówię? Dlatego że jak już będziemy wdrażać te API nasze na produkcję to systemy tam działające w przypadku zrestartowania waszego API, a  na początku wysyłają `SIGTERM` do waszego API. I to w naszym kontekście oznacza że spokojnie funkcja `shutdown` i nie mamy się o co martwić. Natomiast bardzo ważne jest to że systemy produkcyjne domyślnie będą czekać 30 sekund na to aż wasze API po prostu się wyłączy czyli Innymi słowy skrypt `shutdown` powinien zakończyć swoje czynności w maksymalnie 30 sekundach. Generalnie tak będzie dlatego że zamknięcie połączeń, czy innych operacji tak naprawdę jest bardzo bardzo szybkie więc de facto ciężko mi jest podać przykład który spowodowałby że ten `shutdown` będzie trwał dłużej niż 30 sekund. Ale co się stanie jeżeli będzie trwał dłużej niż 30 sekund. Po 30 sekundach wysłany będzie sygnał `SIGKILL` czyli wasze API zostanie ubite natychmiastowo bez możliwości dokończenia dalszych operacji, tak jak to widzieliście na tym przykładzie. Docelowo będziemy wdrażać nasze API na  klastry Kubernetesowe i tam 30 sekund jest domyślne i  z tego cowiem to tego zmienić się nie da.
