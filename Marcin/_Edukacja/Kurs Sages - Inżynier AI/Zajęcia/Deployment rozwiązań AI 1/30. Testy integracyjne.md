...
Przechodzimy sobie teraz do testów integracyjnych które Określone są również jako testy end-2-end.  zapewne domyślacie się o co mąż może chodzić w testach integracyjnych ale zacznijmy od tego żeby sobie to wyjaśnić jak sama nazwa testu wskazuje będziemy sprawdzać pewną integrację albo nawet interakcje Generalnie w testach integracyjnych chodzi o to aby sprawdzić to jak nasze API zachowuje się podczas korzystania z innych serwisów z których korzysta w naszym przypadku są to serwisy związane z bazami danych Redis oraz postgres. Czyli Innymi słowy chcemy sprawdzić czy API poprawnie zapisuje wczytuje dane z tych dwóch baz danych których używamy. Ten Case nasz będzie dosyć prosty jak na testy integracyjny Ale w waszych API która będzie implementować może dojść inna usługa niebazodanowa. Ja na przykład w pracy dodatkowo Zabezpieczam  dokumentacja naszego API w taki sposób że można ją wyświetlić tylko po zalogowaniu się i korzystam z naszych wewnętrznych serwisów odpowiedzialnych za autoryzację w takiej sytuacji no jest to możliwe ale bardzo trudne żeby lokalnie sobie taki serwis autoryzacyjny zdeployować. 

  

W zależności od tego z jakich serwisów dodatkowe będzie wasza API korzystać będzie zależeć trudność pisania testów integracyjnych i w zależności od tej trudności będzie musieli decydować o wyborze konkretnych metod testowania No tak żeby te testy integracyjne przyniosły wam jakąś wartość.
...
2. stubs - albo  inaczej określony jako fejki czyli sztuczne implementacje które pozwalają na interakcje z tymi serwisami Tak jakby one naprawdę były u nas  zaimplementowane
...
W folderze integration stwórzmy sobie dwa pliki,  pierwszy oczywiście fixtures.py  i drugi test decisions bo w ciągle Będziemy operować cały czas na endpoincie decisions.

  

Przejdźmy do fixtures I zacznijmy od zaimportowania niezbędnych  obiektów:
...
Skopiujmy sobie nasze interaiste App z fixtures.py z unit testów. 
...
1. po pierwsze  Każdy test integracyjny będzie będzie wchodził interakcje z bazami danych Dlatego też chciałbym żebyśmy ustawili scoupe fixtures na function. Co to oznacza oznacza to że z każdym wywołaniem Testu nasz klient testowy z API oraz wszelkie obiekty które zaraz do niego dodamy będą się na nowo inicjować co Każdy test jest bardzo ważne bo nie chcielibyśmy żeby wyniki jednego testu wpłynęły na wyniki kolejnych testów one muszą być od siebie niezależne. Ty też oznacza że musimy zmienić `package_mocker` na zwykłego `mocker`.  a z tego mokowania zostawmy sobie dwie linijki tylko związane z tworzeniem połączenia 
...
Teraz zobaczycie połączenie mokowania z tym drugim podejściem czyli użycia subs bądź fake’ów. To co musimy zrobić to po prostu stworzyć sztuczną implementację redisa i post gresa ale oczywiście nie będziemy wynajdywać koła na nowo dlatego że to zostało zrobione przez twórców bibliotek specjalnie stworzonych pod testy integracyjne zacznijmy sobie od redisa.

  

Na potrzeby testów integracyjnych  które wchodzą w interakcję serwisy mamy bibliotekę fake Redis.  to co my zrobimy to zrobimy sobie sztuczny Fik szczur z tą bazą i podamy ją do naszej testowego API:
...
I tak naprawdę robota jest z głowy. Biblioteka `fakeredis`  dostarcza  sztucznej integracja redisa wraz z głównymi jej funkcjonalnościami niestety Są też pewne różnice Natomiast w naszym przypadku one nie będą zbyt istotę tego że My korzystamy z redisa bardzo podstawowym zakresie w bardzo podstawowy sposób,  więc nie będzie z tym żadnego problemu.

  

To co tam zostało to teraz zrobienie podobnej rzeczy ale dla Postgresa. Niestety z podgresem i w sumie jakąkolwiek inną bazą danych która ma w sobie właśnie tabelki schematy i tak dalej będzie troszkę więcej pracy Bo w momencie jej stworzenia musimy ją zainstalować tak jak ona działa na produkcji. W tym celu wykorzystuje bibliotekę pytest_postgres która już jest w importach. Generalnie nie lubię tej biblioteki Bo ona jest po prostu ciężko napisana i używanie jej jest dosyć nieprzyjemne ale niestety nie ma lepszej jestem w wypadku skorzystamy z niej.  niestety stworzenie fejkowego posresa nie jest już takie proste jak tutaj jeden linijką A tu trzeba będzie kilka kilkanaście napisać.
...
Ten obiekt Table schemat to jest po prostu to są po prostu ścieżki do naszych skryptów które tworzą tabele. Będą tam ona teraz potrzebne dlatego że w momencie inicjalizacji bazy będziemy chcieli je podać aby te tabele zostały utworzone.
...
W naszych rogach musimy zdefiniować w jaki sposób będziemy wchodzić w nią w interakcje oraz połączenie z nią. 
...
Ta klasa ta DatabaseJanitor To jest implementacja która wchodzi w interakcję z naszą bazą danych.  następnie  chcę żeby on załadował nasze sql-ki więc korzystam z `janitor.load`. I następnie zwracam połączenie z postgresem. 

  

Słuchajcie Wiem że to może nie być jeszcze do końca jasne ale niestety właśnie Każda biblioteka która pozwala wam na tworzenie takich sztucznych baz danych  No ma swoją własną logikę działania i niestety trzeba się i wszystkich uczyć na nowo więc te wywołanie do tej telewizji w ten sposób na to żeby uruchomić sobie bazę danych postgres korzystając z tej biblioteki. Teraz z PO zdefiniowaniu w ten sposób `fake_postgres`  możemy go to sobie dorzucić  naszego testowego API
...
Czy Reasumując użyliśmy mockowania do tego żeby zasymulować połączenie z sztuczną bazą danych która zostanie uruchomiona tylko muszę na potrzeby testów integracyjnych. I tak mówiłem `scope` jest ustawion na `function`. Dlatego że testy integracyjne będą wchodzić w interakcje z różnymi bazami danych,  z cope fanction w tym przypadku będzie oznaczać to że za każdym razem te bazy będą odtwarzane,  dzięki temu unikniemy  że testy będą od siebie zależne tak po prostu każdy test  będzie rozpoczynał swoje działanie na pustych bazach danych.

  

A teraz  Zobaczmy na własne oczy czy to w ogóle działa. Przejdźmy sobie do `test_decisions.py` Zaimportujmy sobie nasze fikszczur które służyć będą do przeprowadzania testów:
...
Jak widzicie udało się wszystko dostaliśmy responsa ze statusem 200 ale to oczywiście dopiero początek naszego testowania dlatego że teraz musimy upewnić czy faktycznie te dane w Redisie w kongresie są. Ten test będzie troszkę rozbudowany jak sami razem zobaczycie dlatego często integracyjny trzeba sprawdzić dosyć sporo rzeczy.

  

Zawsze może od tego że nie będziemy sprawdzać już zawartości responsa dlatego że wiemy że ona jest poprawna bo mamy po to testy jednostkowe które to sprawdzają dlatego tutaj tylko i wyłącznie upewniamy się że status jest równy 200 to znaczy że Response są poprawnie przetworzony i dalej możemy sprawdzać teraz nasze bazy danych.

  

Wróćmy do main.py i spójrzmy jeszcze raz na logikę działąnia naszego endpointu. Mamy zczytanie danych z redisa, jeśli Tych danych nie ma to wtedy wykonywana jest predykcja modelu i następnie mamy zapisywanie do redisa i zapisywanie do postgresa. Teraz te spanie nam pomogą upewnić się że faktycznie tak się zadziało. Wróćmy do naszych testów i teraz zobaczycie o co chodzi. 
...
Okej udało nam się  rozpatrzeć ścieżkę tą pierwszą czyli wtedy kiedy  kiedy petycja jest związana z modelu teraz musimy jeszcze rozpatrzeć tą drugą ścieżkę czyli wtedy kiedy odpowiedź dostaniemy skecza to proponuję sobie w ramach tego samego testu bo mamy już obiekty z zainicjowane odpowiednio łatwiej nam to będzie przetestować niż robić to w oddzielnym teście. 
...
W przypadku modelu musimy się upewnić teraz że wciąż `call_count`  wynosi 1 to znaczy że funkcja predyk model nie została po raz drugi użyta. 
...
Teraz może troszkę wyjaśnienia ja w swoim implementacji RedisClienta zawarłem już kody które przekształcają wynik z podgresa w postaci zwykłych słownika na trzy obiekty - Wiersz zawiera w sobie `metadata` oraz dane  o `request` oraz `response`.

  

Okej Włączymy sobie testy:
...
Udało się zakończyliśmy nasz pierwszy test integracyjny oczywiście to co rozpatrzyliśmy to jest nasze ścieżki pozytywne czy to trzeba rozpatrzeć to jeszcze oczywiście ścieżki negatywne ale to zostawię wam w ramach ćwiczeń na API który tworzyliście do tej pory. 
...
I zobaczcie udało się Czyli tak naprawdę nawet nie musiałbym definiować tych wszystkich fejków które tutaj zrobiłem. Ale jest jedno ale, uruchommy je drugi raz. Niestety trzeba byłoby zadbać o to żeby te bazy się restartowały ale to dopiero poznamy w następnym zjeździe kiedy zapoznacie się z technologią Dockera. 

  

Generalnie widzicie napisanie testów integracyjnych jest dosyć czasochłonne wymagające ale tak wam powiedziałem to jest inwestycja w przyszłość bo później Podczas jakichkolwiek zmian w waszym API i czy to dobrze że testujecie to naprawdę wyłapie wam wszelkie błędy i bagi które możecie popełnić a jesteśmy ludźmi i takie błędy cały czas będziemy popełniać.
