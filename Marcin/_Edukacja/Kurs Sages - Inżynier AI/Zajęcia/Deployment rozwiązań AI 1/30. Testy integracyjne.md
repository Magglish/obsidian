...
Czy Reasumując użyliśmy mockowania do tego żeby zasymulować połączenie z sztuczną bazą danych która zostanie uruchomiona tylko na potrzeby testów integracyjnych. I tak mówiłem `scope` jest ustawion na `function`. Dlatego że testy integracyjne będą wchodzić w interakcje z różnymi bazami danych,  `scope` fanction w tym przypadku będzie oznaczać to że za każdym razem te bazy będą odtwarzane,  dzięki temu unikniemy  że testy będą od siebie zależne tak po prostu każdy test  będzie rozpoczynał swoje działanie na pustych bazach danych.
