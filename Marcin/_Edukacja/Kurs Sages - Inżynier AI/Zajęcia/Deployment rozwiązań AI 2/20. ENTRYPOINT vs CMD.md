...
Albo jeszcze inny Case:  uruchomimy sobie nasze API żeby  obsługiwało requesty:

  

```bash

docker run -it -p 8080:8080 magglish/inzynier-ai-live-coding:cmd-vs-entrypoint

```

  

 a w drugim terminalu uruchomię ten sam kontener tylko wskaże mu skrypt związany z wysłaniem przykładowego requesta.

  

```bash

docker run --network=host magglish/inzynier-ai-live-coding:cmd-vs-entrypoint scripts/send_example_request.py

```

  

I zadziałało.  niestety w tym przypadku muszę przekazać  argumentu który Nie umawiałem,  nazywa się `--network`,  ale Omówię go dopiero później  przy `docker-compose`, Natomiast ten argument pozwala na to żeby inne kontenery się ze sobą komunikowały. 
