...
1. Stwórzcie obrazy bazowe `python`, `redis`, `postgres`
2. Zaktualizujcie Dockerfile z api, żeby korzystał z nowego bazowego obrazu pythono
3. Przeanalizujcie skrypt `run_redis.sh`
4. Stwórzcie dedykowany Dockerfile w `deploy/docker/redis/api`
5. Zaimplementujcie go
6. Zbudujcie obraz i wyślijcie go do repo `magglish/inzynier-ai-live-coding:postgres`
7. Zmodyfikujcie skrypt `run_redis.sh` aby pobrał nowy obraz i uruchomił go.
8. Usuń zbędny kod (jeśli istnieje) z `run_redis.sh`.
9. Wrzuć dedykowane kontenery do google cloud
10. Wrzuć też API do google cloud
11. Wrzuć też Pythona do google cloud
